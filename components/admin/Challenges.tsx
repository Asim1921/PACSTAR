...
import { Plus, List, Trash2, Rocket, HelpCircle, Eye, EyeOff, Minus, Plus as PlusIcon, Lightbulb, Container, Folder, Upload, FileArchive, Lock, MapPin, Cloud, Code, Camera } from 'lucide-react';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { RadioGroup } from '@/components/ui/RadioGroup';
import { Select } from '@/components/ui/Select';
import { InfoBox } from '@/components/ui/InfoBox';
import { challengeAPI, fileAPI, openStackAPI } from '@/lib/api';
...
type OpenStackProvisioningMode = 'heat_template' | 'snapshot';
type OpenStackTemplateSource = 'paste' | 'upload';
...
export const Challenges: React.FC = () => {
  ...
  const openStackTemplateFileInputRef = useRef<HTMLInputElement>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [editingChallengeId, setEditingChallengeId] = useState<string | null>(null);
  const [challengeListKey, setChallengeListKey] = useState(0); // Force re-render of ChallengeList

  // OpenStack UI state (used only when architecture=openstack)
  const [openStackMode, setOpenStackMode] = useState<OpenStackProvisioningMode>('heat_template');
  const [openStackTemplateSource, setOpenStackTemplateSource] = useState<OpenStackTemplateSource>('paste');
  const [openStackTemplateBody, setOpenStackTemplateBody] = useState<string>('');
  const [openStackTemplateParams, setOpenStackTemplateParams] = useState<string>('{}');
  const [openStackSnapshots, setOpenStackSnapshots] = useState<any[]>([]);
  const [isLoadingSnapshots, setIsLoadingSnapshots] = useState(false);
  const [snapshotImageId, setSnapshotImageId] = useState<string>('');
  const [snapshotFlavor, setSnapshotFlavor] = useState<string>('m1.small');
  const [snapshotNetwork, setSnapshotNetwork] = useState<string>('private');
  const [snapshotExternalNetwork, setSnapshotExternalNetwork] = useState<string>('public');
  const [snapshotAllocateFloatingIp, setSnapshotAllocateFloatingIp] = useState<boolean>(true);
  ...
  // Helper: read a file as text for Heat templates
  ...
  const buildSnapshotHeatTemplate = () => {
    // Generates a Heat template that boots a Nova server from an image/snapshot and optionally allocates a floating IP.
    // Output keys match what challenge_service looks for: server_id, floating_ip (optional).
    return `heat_template_version: 2016-10-14

description: Auto-generated by PACSTAR (snapshot-based OpenStack challenge)
...
parameters:
  ...
  allocate_fip:
    type: string
    description: Whether to allocate a floating IP (true/false)
    default: "true"
  image:
    type: string
    description: Image/Snapshot ID or name to use for the server
  flavor:
    type: string
  ...
  external_network:
    type: string
    description: External network for floating IP allocation (ignored if floating IP is disabled)

conditions:
  allocate_fip_condition: { equals: [ { get_param: allocate_fip }, "true" ] }
...
resources:
  ...
  floating_ip:
    ...
    condition: allocate_fip_condition
    properties:
...
outputs:
  ...
  floating_ip:
    ...
    value: { if: [ allocate_fip_condition, { get_attr: [floating_ip, floating_ip_address] }, "" ] }
`;
  };
  ...
  const loadSnapshots = async () => {
    ...
    try {
      ...
      const list = Array.isArray(result) ? result : (result.snapshots || result.data || []);
      setOpenStackSnapshots(list);
    } catch (e: any) {
      const msg = e?.response?.data?.detail || e?.message || 'Failed to load snapshots';
      showToast(msg, 'error');
    } finally {
  ...
  // When switching to OpenStack snapshot mode, prefetch snapshots.
  useEffect(() => {
    if (formData.architecture === 'openstack' && openStackMode === 'snapshot' && openStackSnapshots.length === 0) {
      loadSnapshots();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [formData.architecture, openStackMode]);
  ...
  const loadChallengeForEdit = async (challengeId: string) => {
    try {
      ...
      // Load OpenStack template fields (if applicable)
      if ((challenge.config?.architecture || (challenge.challenge_category === 'openstack' ? 'openstack' : 'kubernetes')) === 'openstack') {
        setOpenStackMode('heat_template');
        setOpenStackTemplateSource('paste');
        setOpenStackTemplateBody(challenge.config?.heat_template || '');
        setOpenStackTemplateParams(
          challenge.config?.heat_template_parameters
            ? JSON.stringify(challenge.config.heat_template_parameters, null, 2)
            : '{}'
        );
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.challengeMode !== 'static') {
        // OpenStack path (dynamic/multi-flag, architecture=openstack)
        if (formData.architecture === 'openstack') {
          // Parse Heat template parameters JSON
          let parsedHeatParams: Record<string, any> = {};
          try {
            const trimmed = (openStackTemplateParams || '{}').trim();
            parsedHeatParams = trimmed ? JSON.parse(trimmed) : {};
            if (typeof parsedHeatParams !== 'object' || parsedHeatParams === null || Array.isArray(parsedHeatParams)) {
              showToast('OpenStack template parameters must be a valid JSON object', 'error');
              setIsSubmitting(false);
              return;
            }
          } catch (e) {
            showToast('Invalid JSON in OpenStack template parameters', 'error');
            setIsSubmitting(false);
            return;
          }

          let templateBody = (openStackTemplateBody || '').trim();

          // Snapshot mode: generate a template and params
          if (openStackMode === 'snapshot') {
            if (!snapshotImageId) {
              showToast('Please select a snapshot/image for the OpenStack challenge', 'error');
              setIsSubmitting(false);
              return;
            }

            templateBody = buildSnapshotHeatTemplate().trim();
            parsedHeatParams = {
              ...parsedHeatParams,
              image: snapshotImageId,
              flavor: snapshotFlavor || 'm1.small',
              network: snapshotNetwork || 'private',
              external_network: snapshotExternalNetwork || 'public',
              allocate_fip: snapshotAllocateFloatingIp ? 'true' : 'false',
            };
          }

          if (!templateBody) {
            showToast('Please provide a Heat template (paste/upload) or use Snapshot mode', 'error');
            setIsSubmitting(false);
            return;
          }
          ...
          challengeData = {
            ...
            config: {
              ...
              heat_template: templateBody,
              heat_template_parameters: parsedHeatParams,
              flags: formData.challengeMode === 'multi_flag' ? formData.flags : [],
              // Keep flag server / baydrak configs available for dynamic flags if you use them.
              flag_server: {
          ...
          <div>
            ...
            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <Select
                ...
                onChange={(e) => {
                  const nextArch = e.target.value;
                  setFormData((prev) => {
                    // OpenStack doesn't make sense for "static" mode in current UX;
                    // force back to dynamic if user switches architecture to openstack.
                    const nextMode = nextArch === 'openstack' && prev.challengeMode === 'static' ? 'dynamic' : prev.challengeMode;
                    return { ...prev, architecture: nextArch, challengeMode: nextMode };
                  });
                  if (nextArch === 'openstack' && formData.challengeMode === 'static') {
                    setCategory('containerized');
                  }
                }}
                options={[
              ...
              <div className="bg-cyber-900/80 backdrop-blur-xl rounded-2xl shadow-lg border border-neon-green/20 terminal-border p-6 space-y-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-neon-green/20 backdrop-blur-sm rounded-xl flex items-center justify-center border border-neon-green/40">
                    {formData.architecture === 'openstack' ? (
                      <Cloud className="text-neon-green" size={20} />
                    ) : (
                ...
                {formData.architecture === 'openstack' && (
                  <InfoBox
                    type="info"
                    message="OpenStack challenges deploy per-team VMs using a Heat template stored in the challenge config. Teams start their own instance; the backend will surface Floating IP + VNC URL when available."
                  />
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {formData.architecture !== 'openstack' ? (
                      ...
                      <div className="border border-neon-green/20 rounded-xl p-4 space-y-4">
                        <div className="flex items-center gap-2">
                          <Cloud className="text-neon-green" size={16} />
                          <h6 className="text-white font-semibold">OpenStack Provisioning</h6>
                        </div>

                        <RadioGroup
                          options={[
                            { value: 'heat_template', label: 'Heat Template' },
                            { value: 'snapshot', label: 'Snapshot (auto-generate template)' },
                          ]}
                          value={openStackMode}
                          onChange={(value) => setOpenStackMode(value as OpenStackProvisioningMode)}
                        />

                        {openStackMode === 'heat_template' && (
                          <>
                            <RadioGroup
                              options={[
                                { value: 'paste', label: 'Paste YAML' },
                                { value: 'upload', label: 'Upload HOT/YAML' },
                              ]}
                              value={openStackTemplateSource}
                              onChange={(v) => setOpenStackTemplateSource(v as OpenStackTemplateSource)}
                            />

                            {openStackTemplateSource === 'paste' && (
                              <div>
                                <label className="block text-sm font-semibold text-white/90 mb-2">
                                  Heat Template (YAML) *
                                </label>
                                <textarea
                                  value={openStackTemplateBody}
                                  onChange={(e) => setOpenStackTemplateBody(e.target.value)}
                                  rows={10}
                                  placeholder="heat_template_version: 2016-10-14"
                                  className="w-full px-4 py-3 bg-cyber-800/50 border-2 border-neon-green/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:border-neon-green focus:ring-2 focus:ring-neon-green/20 transition-all resize-y font-mono text-sm"
                                />
                              </div>
                            )}

                            {openStackTemplateSource === 'upload' && (
                              <div>
                                <input
                                  ref={openStackTemplateFileInputRef}
                                  type="file"
                                  accept=".yaml,.yml,.json,.template,.hot"
                                  className="hidden"
                                  onChange={async (e) => {
                                    const file = e.target.files?.[0];
                                    if (!file) return;
                                    try {
                                      ...
                                      setOpenStackTemplateBody(text);
                                      showToast('Heat template loaded from file', 'success');
                                    } catch (err: any) {
                                      showToast(err?.message || 'Failed to read template file', 'error');
                                    }
                                  }}
                                />
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  onClick={() => openStackTemplateFileInputRef.current?.click()}
                                >
                                  <Upload size={16} className="mr-2" />
                                  Upload Template File
                                </Button>
                                {openStackTemplateBody?.trim() && (
                                  <p className="text-xs text-white/60 mt-2">
                                    Loaded template content ({openStackTemplateBody.length} chars)
                                  </p>
                                )}
                              </div>
                            )}
                        ...
                        {openStackMode === 'snapshot' && (
                          <>
                            <div className="flex items-center justify-between gap-3">
                              <label className="text-sm font-semibold text-white/90">Snapshot / Image *</label>
                              <Button
                                type="button"
                                variant="outline"
                                size="sm"
                              ...
                              >
                                <Camera size={16} className={`mr-2 ${isLoadingSnapshots ? 'animate-spin' : ''}`} />
                                {isLoadingSnapshots ? 'Loading...' : 'Refresh'}
                              </Button>
                            </div>
                            <select
                              value={snapshotImageId}
                              onChange={(e) => setSnapshotImageId(e.target.value)}
                              className="w-full px-4 py-3 bg-cyber-800/50 border-2 border-neon-green/20 rounded-xl text-white focus:outline-none focus:border-neon-green focus:ring-2 focus:ring-neon-green/20 transition-all"
                            >
                              <option value="" className="bg-cyber-900">-- Select Snapshot --</option>
                              {openStackSnapshots.map((s: any) => (
                                <option key={s.id || s.name} value={s.id || s.name} className="bg-cyber-900">
                                  {s.name || s.id}
                                </option>
                              ))}
                            </select>
                            ...
                            <Input
                              label="Flavor"
                              value={snapshotFlavor}
                              onChange={(e) => setSnapshotFlavor(e.target.value)}
                              placeholder="m1.small"
                            />
                            <Input
                              label="Network"
                              value={snapshotNetwork}
                              onChange={(e) => setSnapshotNetwork(e.target.value)}
                              placeholder="private"
                            />
                            <Input
                              label="External Network (for Floating IP)"
                              value={snapshotExternalNetwork}
                              onChange={(e) => setSnapshotExternalNetwork(e.target.value)}
                              placeholder="public"
                            />
                            ...
                            <label className="flex items-center gap-2 text-sm text-white/80">
                              <input
                                type="checkbox"
                                checked={snapshotAllocateFloatingIp}
                                onChange={(e) => setSnapshotAllocateFloatingIp(e.target.checked)}
                        ...
                        <div>
                          <label className="block text-sm font-semibold text-white/90 mb-2">
                            Heat Template Parameters (JSON)
                          </label>
                          <textarea
                            value={openStackTemplateParams}
                            onChange={(e) => setOpenStackTemplateParams(e.target.value)}
                            rows={6}
                            placeholder='{"param1": "value1"}'
                            className="w-full px-4 py-3 bg-cyber-800/50 border-2 border-neon-green/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:border-neon-green focus:ring-2 focus:ring-neon-green/20 transition-all resize-y font-mono text-sm"
                          />
                          <p className="text-xs text-white/60 mt-2">
                            Stored in DB as <span className="font-mono">config.heat_template_parameters</span>. Backend will also inject per-team <span className="font-mono">server_name</span>.
                          </p>
                        </div>
                      </div>
                    )}
